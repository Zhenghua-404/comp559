...
public:
	Particle(Vec3 pos) : vel(Vec3(0,0,0)), pos(pos), old_pos(pos),acceleration(Vec3(0,0,0)), mass(1), movable(true), accumulated_normal(Vec3(0,0,0)){}
	Particle(){}
	...
	void timeStep()
	{
		if(movable)
		{
			if(verlet){
				...
				old_pos = temp;
				acceleration = Vec3(0,0,0); // acceleration is reset since it HAS been translated into a change in position (and implicitly into velocity)

			}
			if(euler){
				pos = pos + vel*TIME_STEPSIZE2;
				vel += acceleration*TIME_STEPSIZE2;
...
private:
	...
	*/
	...
	void drawTriangle(Particle *p1, Particle *p2, Particle *p3, const Vec3 color)
	{
			...
			Vec3 localObj1 = p1->getNormal().normalized();
			glNormal3fv((GLfloat *) & localObj1);
			glVertex3fv((GLfloat *) &(p1->getPos() ));

			Vec3 localObj2 = p2->getNormal().normalized();
			glNormal3fv((GLfloat *) & localObj2);
			glVertex3fv((GLfloat *) &(p2->getPos() ));

			Vec3 localObj3 = p3->getNormal().normalized();
			glNormal3fv((GLfloat *) & localObj3);
			glVertex3fv((GLfloat *) &(p3->getPos() ));
	}

public:
	double ks = 1.5; //stiffness constant, can tune later
	...
	Cloth(float width, float height, int num_particles_width, int num_particles_height) : num_particles_width(num_particles_width), num_particles_height(num_particles_height)
	{
		...
		linearW.resize((num_particles_width-1)*(num_particles_height-1)*4); //4 w for 2 triangles (wu1, wv1, wu2, wv2)
		widthofParticle = width /(double)num_particles_width;
		heightofParticle = height /(double)num_particles_height;
		// creating particles in a grid of particles from (0,0,0) to (width,-height,0)
		for(int x=0; x<num_particles_width; x++)
		{
			for(int y=0; y<num_particles_height; y++)
			{
				Vec3 pos = Vec3(x * widthofParticle,
								- y* heightofParticle,
								0);
...
//					for(int y=0; y<num_particles_height; y++)
//					{
//						if (x<num_particles_width-3) makeConstraint(getParticle(x,y),getParticle(x+3,y));
//						if (y<num_particles_height-3) makeConstraint(getParticle(x,y),getParticle(x,y+3));
//						if (x<num_particles_width-3 && y<num_particles_height-3) makeConstraint(getParticle(x,y),getParticle(x+3,y+3));
//						if (x<num_particles_width-3 && y<num_particles_height-3) makeConstraint(getParticle(x+3,y),getParticle(x,y+3));			}
//				}
...
//					color = Vec3(1.0f,1.0f,1.0f);
	...
	*/
	...
	void addStretch(int pix, int piy)
	{
		//Customized parameter
		//here let bu, bv be 1 (these are to control shrink/stretch)
		double bu =1;
		double bv =1;
		...
		//getParticles
		Particle* i = getParticle(pix, piy);
		Particle* j = getParticle(pix+1, piy);
		Particle* k = getParticle(pix, piy+1);
		double du1 = widthofParticle;
//		double dv1 = 0; so we don't use them
//		double du2 = 0;
		double dv2 = -heightofParticle;
		double areaTri = abs(widthofParticle * heightofParticle/2);

		Vec3 dx1 = j->getPos()-i->getPos();
		double magX1 = dx1.length();
		Vec3 dx2 = k->getPos()-i->getPos();
		double magX2 = dx2.length();

		Vec3 wu = dx1*dv2;
		Vec3 wv = dx2*du1;

		//fill out the W(x) for
		linearW[4*(piy*(num_particles_width-1)+pix)]=wu;
		linearW[4*(piy*(num_particles_width-1)+pix)+1]=wv;

		Vector2d c1(areaTri*(dv2*magX1-bu),areaTri*(du1*magX2-bv));


		//Now we have C(x) of the triangle, calculate dC(x)/dx for 4 points
		//dC(x)/dxi
		MatrixXd dC1di(3,2);
		MatrixXd dC1dj(3,2);
		MatrixXd dC1dk(3,2);
		...
		dC1di <<	dx1.f[0]/magX1*dv2, dx2.f[0]/magX2*du1,
				dx1.f[1]/magX1*dv2, dx2.f[1]/magX2*du1,
				dx1.f[2]/magX1*dv2, dx2.f[2]/magX2*du1;
		-dC1di;
		dC1dj << dx1.f[0]/magX1*dv2,0,
				dx1.f[1]/magX1*dv2,0,
				dx1.f[2]/magX1*dv2,0;

		dC1dk << 0, dx2.f[0]/magX2*du1,
				0, dx2.f[1]/magX2*du1,
				0, dx2.f[2]/magX2*du1;

		//bottom right triangle

		Particle* g = getParticle(pix+1, piy+1);
//		double du1 = 0;
		double dv1 = heightofParticle;
		double du2 = widthofParticle;
//		double dv2 = 0;

		dx1 = j->getPos()-g->getPos();
		magX1 = dx1.length();
		dx2 = k->getPos()-g->getPos();
		magX2 = dx2.length();
		...
		//fill out the W(x) for
		linearW[4*(piy*(num_particles_width-1)+pix)+2]=wu2;
		linearW[4*(piy*(num_particles_width-1)+pix)+3]=wv2;

		Vector2d c2(areaTri*(dv1*magX2-bu), areaTri*(du2*magX1-bv));

		dC2dg <<	dx2.f[0]/magX2*dv1, dx1.f[0]/magX1*du2,
				dx2.f[1]/magX2*dv1, dx1.f[1]/magX1*du2,
				dx2.f[2]/magX2*dv1, dx1.f[2]/magX1*du2;
		-dC2dg;
		MatrixXd dC2dj(3,2);
		dC2dj << 0,dx1.f[0]/magX1*du2,
				 0,dx1.f[1]/magX1*dv2,
				 0,dx1.f[2]/magX1*dv2;
		MatrixXd dC2dk(3,2);
		dC2dk << dx2.f[0]/magX2*dv1,0,
				 dx2.f[1]/magX2*dv1,0,
				 dx2.f[2]/magX2*dv1,0;
		//mulyiply each deriv by area!

		//now we can add forces to particles : fi = -k * dC(x)/dxi *C(x)
		Vector3d fi = -ks*areaTri*dC1di*c1;
		Vector3d fj = -ks*areaTri*(dC1dj*c1+dC2dj*c2);
		Vector3d fk = -ks*areaTri*(dC1dk*c1+dC2dk*c2);
		Vector3d fg = -ks*areaTri*dC2dg*c2;

		std::cout << "Here is the fi\n" << fi << std::endl;
		std::cout << "Here is the fj\n" << fj << std::endl;
		std::cout << "Here is the fk\n" << fk << std::endl;
		std::cout << "Here is the fg\n" << fg << std::endl;

		(*i).addForce(Vec3(fi(0),fi(1),fi(2)));
		(*j).addForce(Vec3(fj(0),fj(1),fj(2)));
		(*k).addForce(Vec3(fk(0),fk(1),fk(2)));
		(*g).addForce(Vec3(fg(0),fg(1),fg(2)));


	}

	void addShear(int pix, int piy){
		//getParticles
		Particle* i = getParticle(pix, piy);
		Particle* j = getParticle(pix+1, piy);
		Particle* k = getParticle(pix, piy+1);
		...
		double areaTri = abs(widthofParticle * heightofParticle/2);
		//top left triangle
		double c1 = areaTri*(linearW[4*(piy*(num_particles_width-1)+pix)].dot(linearW[4*(piy*(num_particles_width-1)+pix)+1]));
		//dCdxi
		double du1 = widthofParticle;
//		double dv1 = 0; so we don't use them
//		double du2 = 0;
		...
		Vec3 kpos = k->getPos();
		Vec3 gpos = g->getPos();
		double dC1dxi = areaTri*du1*dv2*(2*(ipos.f[0]+ipos.f[1]+ipos.f[2])-(jpos.f[0]+jpos.f[1]+jpos.f[2])-(kpos.f[0]+kpos.f[1]+kpos.f[2]));
		double dC1dyi = areaTri*du1*dv2*(2*ipos.f[1]-jpos.f[1]-kpos.f[1]);
		double dC1dzi = areaTri*du1*dv2*(2*ipos.f[2]-jpos.f[2]-kpos.f[2]);
		double dC1dxj = areaTri*du1*dv2*((kpos.f[0]+kpos.f[1]+kpos.f[2])-(ipos.f[0]+ipos.f[1]+ipos.f[2]));
		double dC1dyj = areaTri*du1*dv2*(kpos.f[1]-ipos.f[1]);
		double dC1dzj = areaTri*du1*dv2*(kpos.f[2]-ipos.f[2]);
		double dC1dxk = areaTri*du1*dv2*((jpos.f[0]+jpos.f[1]+jpos.f[2])-(ipos.f[0]+ipos.f[1]+ipos.f[2]));
		double dC1dyk = areaTri*du1*dv2*(jpos.f[1]-ipos.f[1]);
		double dC1dzk = areaTri*du1*dv2*(jpos.f[2]-ipos.f[2]);

		//now we can add forces to particles : fi = -k * dC(x)/dxi *C(x)
		Vector3d fi = -ks*c1*Vector3d(dC1dxi,dC1dyi,dC1dzi);
		Vector3d fj = -ks*c1*Vector3d(dC1dxj,dC1dyj,dC1dzj);
		Vector3d fk = -ks*c1*Vector3d(dC1dxk,dC1dyk,dC1dzk);


		//bottom right triangle
		double c2 = areaTri*(linearW[4*(piy*(num_particles_width-1)+pix)+2].dot(linearW[4*(piy*(num_particles_width-1)+pix)+3]));
		//dCdxi
		double dv1 = heightofParticle;
		double du2 = widthofParticle;
		double dC2dxg = areaTri*dv1*du2*(2*(gpos.f[0]+gpos.f[1]+gpos.f[2])-(jpos.f[0]+jpos.f[1]+jpos.f[2])-(kpos.f[0]+kpos.f[1]+kpos.f[2]));
		double dC2dyg = areaTri*dv1*du2*(2*gpos.f[1]-jpos.f[1]-kpos.f[1]);
		double dC2dzg = areaTri*dv1*du2*(2*gpos.f[2]-jpos.f[2]-kpos.f[2]);
		double dC2dxj = areaTri*dv1*du2*((kpos.f[0]+kpos.f[1]+kpos.f[2])-(gpos.f[0]+gpos.f[1]+gpos.f[2]));
		double dC2dyj = areaTri*dv1*du2*(kpos.f[1]-gpos.f[1]);
		double dC2dzj = areaTri*dv1*du2*(kpos.f[2]-gpos.f[2]);
		double dC2dxk = areaTri*dv1*du2*((jpos.f[0]+jpos.f[1]+jpos.f[2])-(gpos.f[0]+gpos.f[1]+gpos.f[2]));
		double dC2dyk = areaTri*dv1*du2*(jpos.f[1]-gpos.f[1]);
		double dC2dzk = areaTri*dv1*du2*(jpos.f[2]-gpos.f[2]);

		fj += -ks*c2*Vector3d(dC2dxj,dC2dyj,dC2dzj);
		fk += -ks*c2*Vector3d(dC2dxk,dC2dyk,dC2dzk);
		Vector3d fg = -ks*c2*Vector3d(dC2dxg,dC2dyg,dC2dzg);
...
void display(void)
{
	...
	cloth1.windForce(Vec3(0.0,0.0,0.2)*TIME_STEPSIZE2); // generate some wind each frame
...
int main ( int argc, char** argv )
{
	...
	glutCreateWindow( "Cloth Template from cg.alexandra.dk" );
	init();
